...
class AdvantageEstimator(str, Enum):
    ...
    SCALAR_GRPO_MATCHED = "scalar_grpo_matched"
    """Aggregate components before group normalization; matched FARPO ablation baseline."""
...
def _groupwise_population_standardize(
    values: torch.Tensor,
    index: torch.Tensor,
    eps: float,
    estimator_name: str,
) -> torch.Tensor:
    """Population-standardize scalar values inside each repeated-prompt group."""
    bsz = values.shape[0]
    id2score = defaultdict(list)
    id2mean, id2std = {}, {}

    for i in range(bsz):
        value = values[i].item() if isinstance(values[i], torch.Tensor) else values[i]
        id2score[index[i]].append(value)

    for idx, scores in id2score.items():
        assert len(scores) > 1, f"{estimator_name} needs rollout.n > 1."
        id2mean[idx] = np.mean(scores)
        id2std[idx] = np.std(scores)

    normalized = torch.zeros(bsz, dtype=values.dtype, device=values.device)
    for i in range(bsz):
        std_val = id2std[index[i]]
        if std_val > eps:
            normalized[i] = (values[i] - id2mean[index[i]]) / (std_val + eps)

    return normalized


def _masked_whiten_response_advantage(
    response_advantage: torch.Tensor,
    response_mask: torch.Tensor,
) -> torch.Tensor:
    """Broadcast one scalar per response and apply the shared FARPO postprocessing."""
    response_length = response_mask.shape[1]
    expanded = response_advantage.unsqueeze(-1).expand(-1, response_length) * response_mask
    return VF.masked_whiten(expanded, response_mask) * response_mask


def _require_reward_components(
    reward_components: dict[str, torch.Tensor] | None,
    estimator_name: str,
) -> dict[str, torch.Tensor]:
    if reward_components is None or len(reward_components) == 0:
        raise ValueError(
            f"{estimator_name} requires raw reward_components; refusing to fall back to an unmatched scalar score."
        )
    return reward_components


@register_adv_estimator(AdvantageEstimator.SCALAR_GRPO_MATCHED)
def compute_scalar_grpo_matched_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: torch.Tensor,
    reward_components: dict[str, torch.Tensor] | None = None,
    gdpo_weights: dict[str, float] | None = None,
    eps: float = 1e-6,
    **kwargs,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Matched aggregate-then-normalize baseline for the FARPO estimator ablation.

    This path consumes the same raw components, weights, population standard
    deviation, stabilizer, response broadcast, and masked batch whitening as
    ``compute_gdpo_outcome_advantage``. The only intended difference is order:
    components are aggregated before prompt-group normalization.
    """
    del token_level_rewards, kwargs
    reward_components = _require_reward_components(reward_components, "scalar_grpo_matched")

    first_component = next(iter(reward_components.values()))
    aggregate = torch.zeros_like(first_component)
    for reward_name, reward_values in reward_components.items():
        weight = gdpo_weights.get(reward_name, 1.0) if gdpo_weights is not None else 1.0
        aggregate = aggregate + weight * reward_values

    response_advantage = _groupwise_population_standardize(
        aggregate,
        index=index,
        eps=eps,
        estimator_name="scalar_grpo_matched",
    )
    advantages = _masked_whiten_response_advantage(response_advantage, response_mask)
...
def compute_gdpo_outcome_advantage(
    ...
    **kwargs,
    ...
    GDPO decouples the normalization of each reward component before aggregation,
    which preserves more fine-grained distinctions between different reward combinations.

    Reference: GDPO paper (https://arxiv.org/abs/2601.05242)

    Algorithm:
    1. For each reward component r_k, compute group-wise normalized advantage:
       A_k^(i,j) = (r_k^(i,j) - mean{r_k^(i,1),...,r_k^(i,G)}) / std{r_k^(i,1),...,r_k^(i,G)}
    2. Sum the normalized advantages (optionally with weights):
       A_sum^(i,j) = w_1 * A_1^(i,j) + ... + w_n * A_n^(i,j)
    3. Apply batch-wise normalization (following official implementation):
       Â_sum = masked_whiten(A_sum, response_mask)

    Args:
        token_level_rewards: `(torch.Tensor)`
            shape: (bs, response_length); used for dtype/device alignment
        response_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        index: `(torch.Tensor)`
            shape: (bs,) - group index for each sample (samples with same index are from same prompt)
        reward_components: `(dict[str, torch.Tensor])`
            Dictionary mapping reward names to their values, shape: (bs,). This
            input is mandatory so the estimator cannot silently become an
            unmatched scalar baseline.
        gdpo_weights: `(dict[str, float])`
            Optional weights for each reward component. If None, equal weights (1.0) are used.
        eps: `(float)`
            epsilon value to avoid division by zero
    ...
    reward_components = _require_reward_components(reward_components, "gdpo")

    # GDPO/FARPO: Normalize each reward component separately (group-wise)
    normalized_advantages = {}

    for reward_name, reward_values in reward_components.items():
        normalized_advantages[reward_name] = _groupwise_population_standardize(
            reward_values,
            index=index,
            eps=eps,
            estimator_name=f"GDPO reward '{reward_name}'",
        )

    # Sum the normalized advantages with optional weights
    # A_sum = w_1 * A_1 + w_2 * A_2 + ... + w_n * A_n
    advantage_sum = torch.zeros(bsz, dtype=token_level_rewards.dtype, device=token_level_rewards.device)

    for reward_name, norm_adv in normalized_advantages.items():
        weight = 1.0
        if gdpo_weights is not None and reward_name in gdpo_weights:
            weight = gdpo_weights[reward_name]
        advantage_sum = advantage_sum + weight * norm_adv

    # Apply the same response broadcast and batch whitening as the matched scalar baseline.
    advantages = _masked_whiten_response_advantage(advantage_sum, response_mask)
